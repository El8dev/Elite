...
export const SiteHeader: React.FC = () => {
  ...
  // Close the drawer when the viewport grows past the CSS breakpoint (860px),
  // when the route changes, or on Escape. Lock page scroll while it is open.
  useEffect(() => {
    if (!mobileMenuOpen) return;
    const onResize = () => { if (window.innerWidth > 860) setMobileMenuOpen(false); };
  ...
  return (
    <>
      <header className={`site ${isScrolled ? 'scrolled' : ''}`}>
        <div className="nav-row">
          ...
          <div className="nav-actions">
            ...
            <button className="burger" onClick={() => setMobileMenuOpen(true)} aria-label="Open Menu" aria-expanded={mobileMenuOpen} aria-controls="mobile-drawer">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Mobile Drawer (always mounted so the close transition can play) */}
      <div
        id="mobile-drawer" className={`mobile-drawer ${mobileMenuOpen ? 'open' : ''}`}
        aria-hidden={!mobileMenuOpen}
        onClick={(e) => { if (e.target === e.currentTarget) setMobileMenuOpen(false); }}
      >
        <div className={`mobile-drawer__panel ${mobileMenuOpen ? 'open' : ''}`} role="dialog" aria-modal="true" aria-label={t('nav.home')}>
            <button className="mobile-close" onClick={() => setMobileMenuOpen(false)} aria-label="Close Menu" tabIndex={mobileMenuOpen ? 0 : -1}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
