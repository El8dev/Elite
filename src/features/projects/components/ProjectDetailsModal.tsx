          ...
          <div className="p-6 md:p-8 flex-1 md:col-span-6 flex flex-col justify-between overflow-y-auto md:max-h-[85vh]">
            <div>
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                <div>
                  <h2 className="text-2xl md:text-3xl font-bold text-foreground font-outfit">{project.title}</h2>
                  <div className="flex gap-4 mt-3">
                    ...
                    {project.repoUrl && (
                      <a href={project.repoUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors font-outfit">
                ...
                <div className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.techStack.map(tech => (
                      <span key={tech} className="px-3 py-1 bg-secondary border border-border text-purple-700 dark:text-purple-300 text-xs font-medium rounded-md font-outfit">
                        {tech}
                      </span>
                    ))}
                  </div>
                      ...
                      return (
                        <Tooltip key={c.id || c.profile_id}>
                          ...
                          <TooltipContent className="bg-popover border border-border text-popover-foreground p-2.5 rounded-lg shadow-xl font-outfit z-[100]">
                            ...
                            <p className="text-xs md:text-sm text-purple-600 dark:text-purple-400 font-medium">{role}</p>
                          </TooltipContent>
                        </Tooltip>
                      );
                    })}
