...
export const ContactAgencyModal: React.FC<ContactAgencyModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formsubmit.co/ajax/el8dev@gmail.com", {
        method: "POST",
        headers: { 
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2">
                          ...
                          <input name="name" required type="text" className={`w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500/50 transition-colors text-sm ${isRTL ? 'font-alexandria' : 'font-outfit'}`} placeholder={t('contact.name_placeholder')} />
                        </div>
                        <div className="space-y-2">
                          <label className={`text-xs font-semibold text-white/70 ${isRTL ? 'font-alexandria' : 'font-outfit'}`}>{t('contact.email')}</label>
                          <input name="email" required type="email" className={`w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500/50 transition-colors text-sm ${isRTL ? 'font-alexandria' : 'font-outfit'}`} placeholder="email@example.com" />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <label className={`text-xs font-semibold text-white/70 ${isRTL ? 'font-alexandria' : 'font-outfit'}`}>{isRTL ? 'رقم الهاتف' : 'Phone Number'}</label>
                        <input name="phone" required type="tel" className={`w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500/50 transition-colors text-sm ${isRTL ? 'font-alexandria' : 'font-outfit'}`} placeholder={isRTL ? '+964 ...' : '+1 ...'} />
                      </div>

                      <div className="space-y-2">
                        <label className={`text-xs font-semibold text-white/70 ${isRTL ? 'font-alexandria' : 'font-outfit'}`}>{t('contact.project_type')}</label>
                        <select name="project_type" required className={`w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500/50 transition-colors text-sm appearance-none ${isRTL ? 'font-alexandria' : 'font-outfit'}`}>
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea name="details" required rows={4} className={`w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500/50 transition-colors text-sm resize-none ${isRTL ? 'font-alexandria' : 'font-outfit'}`} placeholder={t('contact.details_placeholder')} />
                      </div>
